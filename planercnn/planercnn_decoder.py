...
class MaskRCNN(nn.Module):
    ...
    def __init__(self, config, resnet_enc , model_dir='test'):
        """
        ...
        self.set_log_dir()
        self.build(config=config, resnet_enc= resnet_enc)
        self.initialize_weights()
        self.loss_history = []
        self.val_loss_history = []

    def build(self, config, resnet_enc):
        ...
        C1, C2, C3, C4, C5 = [None,resnet_enc.layer1,resnet_enc.layer2,resnet_enc.layer3,resnet_enc.layer4]
    ...
    def forward(self, planer_input ,layer_1, layer_2, layer_3, layer_4):
        
        
        input=planer_input['input']
        mode=planer_input['mode']
        use_nms = planer_input.get('use_nms',1)
        use_refinement = planer_input.get('use_refinement',False)
        return_feature_map = planer_input.get('return_feature_map',False)
